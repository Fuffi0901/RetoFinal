...
public class VverAdmin extends JDialog implements ActionListener{
	...
	public VverAdmin(Inicio_Sesion inicio_Sesion, boolean b, Dao dao) {
		...
		btnModificar_1.setFont(new Font("Arial", Font.BOLD, 20));
		btnModificar_1.setBounds(1039, 90, 187, 61);
		padArtista.add(btnModificar_1);
	...
	private void cargarDatosArtista() {
		if(comboBorrarArt.getSelectedIndex()!=-1) {
			...
			textNombreArt.setText(artistas.get(pos).getNombrePersona());
			textApellidoArt.setText(artistas.get(pos).getApellidoPersona());
			textPaisArt.setText(""+(artistas.get(pos).getFechaLan()));
			textEdadArt.setText(""+(artistas.get(pos).getFechaLan()));
			textNombreArtisticoArt.setText(""+(artistas.get(pos).getFechaLan()));
			comboEstiloArt.setText();			
		}else {
